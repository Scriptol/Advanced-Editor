...
function sendToInterface(a) {
    event.sender.send("interface", JSON.stringify(a));
}
...
function getdir(thepath, params)
{
  print("Getdir currentpath request: " + thepath);
  ...
  fs.readdir(thepath, function(err, arrfiles)
  {
    ...
    if(arrfiles==undefined) return;  
      ...
      fs.realpath(thepath, "", function (err, resolvedPath) {
          ...
          event.sender.send("filelist", data)
...
function fillBox(params)
{
  ...
  boxpath = path.join(process.resourcesPath, boxpath)
...
{ 
    ...
    function(newname) {
        ...
        var p = path.join(tpath, newname);
        var err = fs.mkdirSync(p, '0777', function(err){
        if(err) 
...
function deletelist(params)
{
  ...
  event.sender.send("interface", JSON.stringify({"type":'status', 'content' : message }));
}
...
ipcMain.on("startPrompt", (event, data) => {
  event.returnValue = JSON.stringify(promptOptions, null, '')
})

ipcMain.on("endPrompt", (event, data) => {
  promptWindow.hide();
  promptResult = data
})

function promptModal(parent, options, cb) {
  ...
  var pp = path.join(process.resourcesPath, "prompt.html")
  promptWindow.loadURL(pp)
...
function copyRenSub(params) {
  var source = currpath[params.source];
  var target = currpath[params.target];
  var sourcepath = path.join(source, params.oldname);
  var targetpath = path.join(target, params.newname);
    if(sourcepath == targetpath)  {
    messageToInterface("Can't copy a file over itself!");
		return;
  }
  ...
  event.sender.send("interface", JSON.stringify({
    "type":'status',  
    'content': sourcepath + " copied to " + targetpath 
  } ));
  event.sender.send("interface", JSON.stringify({
    "type":'notification',  
    'action': 'update', 
  ...
  for(var i = 0; i < flist.length; i++)
  {
    ...
    if(stats.isFile())
    {
      ...
      if(newer) {
        var info = "Copying "+ sourcepath + " to " + target;
        console.log(info);
        event.sender.send("interface", JSON.stringify({ "type":"status", "content": info } ));
        copySync(sourcepath, targetpath);
        total++;
      }  
    }
...
function synchronize(params)
{
  ...
  sendToInterface({ "type":"notification","action": "update", "target" : params.target });    
...
function getContent(params)
{
	...
	if(!here) {
        ...
        if(params.project != null) {
            console.log("Loading project anyway...");
            sendToInterface({"type":"openPrj", 'project':params.project});
        }
    ...
    sendToInterface({"type":'status', 'content' : fullpath });
}


function openProject(params)
{
	var fullpath = canonicalPath(params.name);
	var here = fs.existsSync(fullpath);
	if(!here) {
        ...
        if(params.mode == true) {
            var a = {
                "type":"confirm",
                "question" :"Project file '" +fullpath+ "' not found. Remove from list?",
                "data": {        
                    "command": "delprj",
                    "path" : fullpath
                }
...
function savefile(params)
{
  ...
  if(!params.overwrite)
  {
    if(fs.existsSync(fullpath))
    {
      ...
      event.sender.send("editor", JSON.stringify({
                'action':'confirm', 
    ...
    if(params.convert) {   
        var ext=path.extname(params.origin)
        if(ext == ".md") {    
            let protoMarkup = new MarkdownIt();
            content = protoMarkup.render(params.content)
        } 
    }      
    
    var err = fs.writeFileSync(fullpath, content);    
    var message = (err ? 'Not saved' : 'Saved') + ' into ' + fullpath;
    print(message);
    if(err) {
      event.sender.send("interface", JSON.stringify({"type":'editor',  'message': message }));
    }
    else { 
      event.sender.send("interface", JSON.stringify({"type":'status', 'content' : message }));
      event.sender.send("interface", JSON.stringify({"type":'saved' }));
    }
}
...
function saveSysFile(params)
{
    ...
    print("Save system file " + fullpath);
...
function infosub(basepath)
{
  ...
  for(var i = 0; i < selection.length; i++)
  {
    ...
    //print("next file to test " + nextentry);
    if(stats.isDirectory())  {
       infosub(nextentry);
    }
    else
...
function loadPage(filename, openflag) {
    if(viewWindow == undefined || viewWindow == null) {
        if(!openflag) return;   // from save command 
        explorerWindow();
    }
    ...
    if(ext == ".md") {
        ...
        var temp = path.join(process.resourcesPath, 'aedit'+ new String(Date.now()) + ".tmp")
...
{
    loadPage(params.path, params.open);
}
...
function archive(params)
{
  ...
  if(command.charAt(0) != "/" && command.charAt(1) != ':')  
    command = path.join(process.resourcesPath, command)
  ...
  event.sender.send("interface", JSON.stringify({
      "type":'notification', 'action': 'update', 'target' : params.target 
  }));
}
...
function viewzip(params)
{
	...
	event.sender.send("filelist", JSON.stringify({
        "type":'dirdata', 
        'path': zipname,
        'list' : dirlist,
        'target' : params.target,
...
function unzip(params)
{
  ...
  print("Unzip " + zipname + " to " + currpath[params.target] + " overwrite="+ overwrite);
  var zipfile = new zip(zipname);
...
function progressOn() {
  event.sender.send("interface", JSON.stringify( { "type":"mouse",  'pointer' : 'progress' } ));
}

function progressOff(s) {
  event.sender.send("interface", JSON.stringify( { "type":"mouse",  'pointer' : 'auto' } ));
}
...
exports.shell = function(params)
{
	...
	case 'getdir':
        ...
        getdir(currpath[target], params);
        break;
    case 'saveas':
        subdir(params.path, target);
        getdir(params.path, params);  
        break;
  ...
  case 'sync':
        ...
        break;    
  case 'copyover':
        copyIf(params.path, target, false); // do not test if exists
        break;
  case 'copyzip':
        copyZipOver(command);
        break;      
