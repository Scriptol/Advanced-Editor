...
function getdir(websocket, fs, thepath, params)
{
  ...
  if(!result) {
      var oldpath = thepath;
      thepath = "/";
      result = fs.existsSync(thepath);
      if(!result) {
        messageToInterface(websocket, "Dir not found " + oldpath);
        return;
    }
  }    
  ...
  fs.readdir(thepath, function(err, arrfiles)
    ...
    console.log("Path " + thepath+ ", " + arrfiles.length + " files");
...
function rename(websocket, fs, params)
{
  try {
    fs.renameSync(params.oldname, params.newname);
     ...
     messageToInterface(websocket, "Enable to rename " + params.oldname + " to " + params.newname + " " + e);
  }
}
...
function getContent(websocket, fs, params)
{
	...
	if(!here) {
        ...
        if(params.project != null) {
            ...
            websocket.send(JSON.stringify({"type":"openPrj",
...
// Load project file and then active file in project
...
function savefile(websocket, fs, params)
{
 ...
 print("convert " + params.convert)   
  ...
  var message = (err ? 'Not saved ' + err : 'Saved') + ' into ' + fullpath;
  console.log(message);
  if(err)
      websocket.send(JSON.stringify({"type":'editor',  'message': message }));
  else { 
      websocket.send(JSON.stringify({"type":'status', 'content' : message }));
      websocket.send(JSON.stringify({"type":'saved' }));    
  }
}
...
function saveSysFile(websocket, fs, params)
{
    ...
    result = (result ? 'NOT saved' : 'saved')
    var message = "System file " + result  + ' into ' + fullpath;
    console.log(message);
}
...
function loadInBrowser(filename, cb)
{
...
exports.shell = function(websocket, fs, params)
{
	...
	case 'viewtext':
		...
		break;
	case 'cdialog':
		saveConfirm(websocket, fs, params);
		break;            
	case 'save':
